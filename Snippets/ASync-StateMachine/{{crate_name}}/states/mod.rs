...
pub(crate) trait {{camel_name}}StateFunctions {
    async fn entry(&mut self, _shared: &mut StateShared) -> Option<{{camel_name}}State> {
        None
    }
    async fn exit(&mut self, _shared: &mut StateShared) {}

    #[allow(unused)]
    async fn input(&mut self, _shared: &mut StateShared, _c: char) -> Option<{{camel_name}}State> {
        None
    }
}
...
pub(crate) async fn transition(
    ...
    while let Some(new_state) = next_state {
        ...
        state.exit(shared).await;
        *state = new_state;
        next_state = state.entry(shared).await;
    }
}
